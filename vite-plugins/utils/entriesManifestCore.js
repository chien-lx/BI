...
export function scanProjectEntries(projectRoot, groups = DEFAULT_GROUPS) {
  ...
  for (const group of groups) {
    ...
    for (const name of names) {
      ...
      // 扫描二级目录（支持直接子目录或 pages/ 子目录）
      const subDir = path.join(groupDir, name);
      const subNames = fs
      ...
      for (const subName of subNames) {
        const subJsEntry = path.join(subDir, subName, 'index.tsx');
        if (fs.existsSync(subJsEntry)) {
          const key = `${group}/${name}/${subName}`;
          items[key] = {
            group,
            name: `${name}/${subName}`,
            js: toPosixPath(path.relative(projectRoot, subJsEntry)),
            html: toPosixPath(path.relative(projectRoot, path.join(subDir, subName, 'index.html'))),
          };
          continue;
        }

        // 支持 pages/ 作为中间层：group/name/pages/xxx/index.tsx
        if (subName === 'pages') {
          const pagesDir = path.join(subDir, subName);
          const pageNames = fs
          ...
          for (const pageName of pageNames) {
            const pageJsEntry = path.join(pagesDir, pageName, 'index.tsx');
            if (!fs.existsSync(pageJsEntry)) continue;
            const key = `${group}/${name}/${pageName}`;
            items[key] = {
              group,
              name: `${name}/${pageName}`,
              js: toPosixPath(path.relative(projectRoot, pageJsEntry)),
              html: toPosixPath(path.relative(projectRoot, path.join(pagesDir, pageName, 'index.html'))),
            };
          }
        }
      }
